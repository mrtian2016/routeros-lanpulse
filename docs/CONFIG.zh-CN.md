...
节点是纯声明式的，`x/y/w/h/name/sub`；`core = true` 画粗边框，
`group = true` 是分组标题，组员（WireGuard peers、分支站点、内网 Top 设备……）
的位置由它的 `x/y` 和 `gap` 逐行推出 —— `y = 340, gap = 24` 的组，第三个组员就在
`y = 388`。画布按实际画出来的内容自动定尺寸，不用维护 viewBox。

删掉一个节点，连到它的线会一起消失 —— 所以想简化拓扑就直接删。

**唯一需要留意的排布规则**：一个组所在的横带，要和喂它数据的那个节点保持同样的上下顺序。
`lan` 在 `edge` 上面，所以 `lang`（由 `lan` 喂）占上面那条带，`ing` / `brg` / `tsg`
（都由 `edge` 喂）排在下面。把 edge 的组放到 lan 的组上面，它的连线就得掉头往上穿过
整张图 —— 默认布局以前正是这个样子。
